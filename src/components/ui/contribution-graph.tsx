...
const getMonthLabels = (
  ...
  return weeks
    .reduce<MonthLabel[]>((labels, week, weekIndex) => {
      ...
      if (!firstActivity) {
        // Just return current labels if week is empty
        return labels
      }
...
export const ContributionGraph = ({
  ...
  return (
    ...
    >
      <div
        className={cn("flex w-full flex-col gap-2 items-center", className)}
...
export const ContributionGraphCalendar = ({
  ...
  return (
    <div
      className={cn("max-w-full overflow-hidden w-full flex justify-center", className)}
      {...props}
    >
      <svg
...
export const ContributionGraphFooter = ({
  ...
  const { width } = useContributionGraph()
  
  return (
    <div
      className={cn(
        "flex flex-wrap justify-between w-full gap-1 whitespace-nowrap sm:gap-x-4",
        className
      )}
      style={{ maxWidth: width }}
      {...props}
